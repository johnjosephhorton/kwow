...
% JJH: I just mean that they are not representative. 
...
%TODO: all else equal, a 1 standard deviation increase in our social knowledge index is associated with TK. 
% Created an Issue here for you :) 
...
% JJH: Unfortunately, no, but I think I explained the issue a bit better. 
...
%What is an "information intervention"? Maybe this deserves a few more words
% JJH: An econ audience will know what this means - it just means telling people stuff :). 
...
%I'm not sure what's going on here. I tihnk we only need to talk about one of the restrictions.
% JJH: I wanted to exclude both observations where the person didn't know what they the job was (obviously) and also jobs with a low overall knowledge rate, since the people who say they do know are more likely to be mistaken. 
...
%I don't understand: why didn't you ask for a "10yr" prediction? I think this needs another sentence of justification.
% Because 10 years is too short --- I figured too people would just pool on "Stay the same" 
...
% JJH: Created an issue for you :). 

%I wonder if you ran the regression with a different mapping, let's say 0, 1, 7.5, 10 if anything would change.
...
% With FE there will not be an intercept, at least with this way of doing it. Is it more common in stats to pull out the mean of the FEs and call it the intercept?  
%Oops - duh..
...
\includegraphics[width = 0.90\linewidth]{./plots/predicted_v_actual.pdf} 
\\
...
%%I'm wondering: if you run the regression just on the occupations with the largest error (the managers and whatnot), would you see an effect? It's possible the effect is masked by a ton of noise. Either way, we should report the result. If it's negative, it makes our argument stronger.
...
%I'm sure there's literature we can pull in here on the "value of networking" or whatever it's called technically.
...
% Yeah - but the typo was in the actually survey, so I think it needs to stay. 
\begin{figure}
\centering
...
%Really is a shame we didn't have Javascript converting hourly wage to annual salary... That may be the cause of some of the errors as you mentioned. If I were a reviewer, I might ask you to rerun it.
